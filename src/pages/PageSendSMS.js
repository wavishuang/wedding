  ...
  const sendEmail = (data) => {
    ...
    setTimeout(() => {
      ...
      async function triggerSend() {
        const sendEmailInviteCard = await api_send_sms_invite_card(formData);
  ...
  const renderEmailList = (list) => {
    ...
    if(list && list.length > 0) {
      const lists = list.map((item, index) => {
        ...
        let sendStatus = item.SMSSendDateTime 
          ? <small>{item.SMSSendDateTime.substr(0,19).replace("T", " ")}已寄送</small>
          : state;

        let sendBtn = item.SMSSendDateTime
          ? <button className="btn sendButtons-retry btn-block px-0 text-light btn-padding" onClick={() => sendEmail(item)}>重新發送</button>
          : <button className="btn sendButtons btn-block px-0 text-light btn-padding" onClick={() => sendEmail(item)}>發送 SMS 圖文簡訊邀請函</button>
          ...
          <Row>
            ...
            <Col xs={11} md={8} className="mx-auto mbr-form bg-main-color border-radius-15">
              <div className="dragArea form-row mt-30">
                <Col xs={12}>
                  <h4 className="mbr-fonts-style display-5 text-center text-main-color">SMS 文字簡訊邀請函發送</h4>
                </Col>
                <Col xs={12}>
                  <hr />
                </Col>
