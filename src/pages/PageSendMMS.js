...
const PageSendMMS = function() {
  ...
  const UpdateProgress = async () => {
    ...
    if(postManProgress.data && postManProgress.data.Msg === 'OK') {
      ...
      console.log("resPostManProgress:", resPostManProgress);
  ...
  const sendEmail = (data) => {
    ...
    setTimeout(() => {
      ...
      async function triggerSend() {
        const sendEmailInviteCard = await api_send_mms_invite_card(formData);
  ...
  */
  ...
  const renderEmailList = (list) => {
    console.log("isBatchProcess", isBatchProcess);
    if(list && list.length > 0) {
      const lists = list.map((item, index) => {
        ...
        let sendStatus = item.MMSSendDateTime 
          ? <small>{item.MMSSendDateTime.substr(0,19).replace("T", " ")}已寄送</small>
          : state;

        let sendBtn = item.MMSSendDateTime
          ? <button className="btn sendButtons-retry btn-block px-0 text-light btn-padding" onClick={() => sendEmail(item)}>重新發送</button>
          : <button className="btn sendButtons btn-block px-0 text-light btn-padding" onClick={() => sendEmail(item)}>發送 MMS 圖文簡訊邀請函</button>
  ...
  return (
    <Fragment>
      <HeaderDiv />
      <ButtonSendInvitation isBatchProcess={true} sendAll={null} isSticker={false} />
      
      <section className="form cid-rLQ7009Pot pt-5">
        <Container>
          <Row>
            <Col xs={{ span: 10, offset: 1 }} md={{ span: 5, offset: 4}} className="form-group">
              {introImages && introImages[5] && introImages[5].Image && 
              <img src={`http://backend.wedding-pass.com/ERPUpload/4878/${introImages[5].Image}`} className="img-fluid" />}
            </Col>
          
            <Col xs={11} md={8} className="mx-auto mbr-form bg-main-color border-radius-15">
              <div className="dragArea form-row mt-30">
                <Col xs={12}>
                  <h4 className="mbr-fonts-style display-5 text-center text-main-color">MMS 圖文簡訊邀請函發送</h4>
                </Col>
                <Col xs={12}>
                  <hr />
                </Col>
