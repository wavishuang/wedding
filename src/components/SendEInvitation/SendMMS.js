    ...
    if(dtlist && dtlist.length > 0) {
      lists = dtlist.map((item, index) => {
        ...
        let sendBtn = item.MMSSendDateTime
          ...
          : <button className="btn sendButtons btn-block text-light btn-padding" onClick={() => sendEmail(item)}>發送 MMS 圖文簡訊邀請函</button>
  ...
  return (
    <section className="features1 cid-rX4jzrRcmX bg-color-pink">
      <section className="form cid-rLQ7009Pot p-0 bg-color-transparent">
        <div className="fixed-title-img" style={titleImg(5)}></div>
